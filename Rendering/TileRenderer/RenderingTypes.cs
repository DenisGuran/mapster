...
public struct PopulatedPlace : BaseShape
{
    ...
    public static bool ShouldBePopulatedPlace(MapFeatureData feature)
    {
        ...
        foreach (var entry in feature.Properties)
            if (entry.Key == PropertyKeys.Place && Enum.TryParse<PopulatedPlaceValues>(entry.Value, true, out var placeValues))
            {
                return true;
            }
        return false;
...
public struct Border : BaseShape
{
    ...
    public static bool ShouldBeBorder(MapFeatureData feature)
    {
        ...
        foreach (var entry in feature.Properties)
        {
            switch (entry.Key)
            {
                case PropertyKeys.Boundary when Enum.TryParse<BoundaryValues>(entry.Value, true, out var boundaryValue):
                    {
                        if (boundaryValue == BoundaryValues.Administrative)
                        {
                            foundBoundary = true;
                        }
                ...
                case PropertyKeys.Admin_Level when entry.Value == "2":
                    foundLevel = true;
                    break;
            }
